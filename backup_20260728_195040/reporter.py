...
class ReportGenerator:
    ...
    def generate_admin(self) -> Dict[str, Any]:
        ...
        report["_raw_data"] = {
            ...
            "tech_stack_impact": self.data.get("tech_stack_impact", {}),
            "business_type": self.data.get("business_type", {}),
            "all_checks": self.data.get("all_checks", {}),
    ...
    # ════════════════════════════════════════════════════════════════════════
    #  FORWARDABLE REPORT — Polished markdown for sharing with customers
    # ════════════════════════════════════════════════════════════════════════

    def generate_forwardable_report(self) -> str:
        """Generate a polished markdown report suitable for forwarding to customers."""
        admin = self.generate_admin()
        scores = admin.get("scores", {})
        readiness = scores.get("readiness_score", 0)
        severity = admin.get("severity", {})
        six_scores = admin.get("six_scores", {})
        url = admin.get("url", self.url)

        lines = []
        lines.append("# Revenue Readiness Report")
        lines.append(f"**Site:** {url}")
        lines.append(f"**Date:** {admin.get('timestamp', datetime.utcnow().isoformat())}")
        lines.append("")
        lines.append("---")
        lines.append("")
        ...
        if visible:
            for i, f in enumerate(visible[:5], 1):
                item = f.get("human_name", f.get("item", "Unknown")).replace("_", " ").title()
                sev = f.get("severity", "medium").upper()
                one_liner = f.get("one_liner", "")
                lines.append(f"{i}. **{item}** — {sev}")
                if one_liner:
        ...
        if fix_steps:
            ...
            for f in fix_steps[:3]:
                item = f.get("item", "Unknown").replace("_", " ").title()
        ...
        if comp and comp.get("competitor_count", 0) > 0:
            ...
            lines.append(f"**Competitors Analyzed:** {comp.get('competitor_count', 0)}")
            lines.append(f"**Gap Score:** {comp.get('gap_score', 0)}/100")
            missing = comp.get("aggregate_missing_features", [])
            if missing:
                lines.append("**Features competitors have that you may be missing:**")
        ...
        if teaser:
            ...
            lines.append("## Revenue Exposure")
            lines.append(f"**Estimated Annual Exposure:** ${cons.get('annual_exposure', 0):,.2f}")
        ...
        if pred:
            ...
            lines.append(f"- 3 months: {pred.get('3', 0)}/100")
            lines.append(f"- 6 months: {pred.get('6', 0)}/100")
        ...
        if roadmap:
            ...
            for week in roadmap[:2]:
                lines.append(f"### {week.get('week', 'Week')} — {week.get('focus', '')}")
                for item in week.get("items", [])[:2]:
                    lines.append(f"- {item.get('item', '')}")
                lines.append("")

        lines.append("---")
        lines.append("")
        lines.append(FORWARDABLE_REPORT_FOOTER)
        lines.append("")
        lines.append("*This report was generated by the Revenue Readiness Scanner.*")
