  ...
  Generated by RRS Report Generator &bull; {datetime.now().strftime("%Y-%m-%d %H:%M")}
    ...
    def _render_admin_html(self, admin: Dict[str, Any]) -> str:
        ...
        for name, val in scores.items():
            html += f"<tr><td>{name}</td><td>{val}/100</td></tr>"
        html += "</table>"
        ...
        if comp and comp.get("competitors"):
            ...
            for c in comp.get("competitors", []):
                ...
                html += f"<p><strong>{c.get('domain', 'Unknown')}</strong><br>"
                html += f"Missing features: {', '.join(missing) if missing else 'None — you match or exceed.'}</p>"
