...
async def verify_admin(
  ...
  if credentials.scheme.lower() != "bearer":
    raise HTTPException(
        status_code=401, detail="Invalid auth scheme. Use Bearer."
    )
  ...
  if any(domain in clean_url for domain in RESTRICTED_DOMAINS):
    raise HTTPException(
        ...
        detail=(
            ...
            "you the existential crisis. Go fix your client's conversion funnel"
            " instead."
        ),
    )
...
class ScanRequest(BaseModel):
  ...
  pass_id: Optional[
      str
  ] = None  # If provided, authenticates against TierManager CSV
...
async def scan(
  ...
  """
  # 1. Normalize and check guardrail[cite: 2]
  ...
  # 2. Authenticate Pass ID or assign Free tier preview[cite: 2]
  client_tier = 3  # Default preview tier for free scans
  if request.pass_id:
    manager = TierManager()
    authorized, client_tier, auth_message = manager.authorize_scan(
        request.pass_id, target_url
    )
    if not authorized:
      raise HTTPException(
          status_code=403, detail=f"Pass Authorization Failed: {auth_message}"
      )

  # 3. Execute Async Scraper (Awaited to fix the loop crash)[cite: 2]
  scraper = SiteScraper(headless=True)
  raw_results = await scraper.scrape_url(target_url)

  # 4. Score External Metrics[cite: 2]
  scorer = ExternalScorer()
  final_checkpoint_results = scorer.enhance_checkpoint_results(
      raw_results, target_url
  )

  # 5. Dynamic Matrix Blueprint Calculation[cite: 2]
  ...
  if report_payload.get("status") != "CALCULATIONS_COMPLETE":
    raise HTTPException(
        status_code=500,
        detail=report_payload.get("message", "Calculation error."),
    )

  # 6. Generate Master Markdown Report[cite: 2]
  ...
  # Append free upsell banner if scanning without an elite pass[cite: 2]
  if not request.pass_id:
    markdown_output += "\n\n---\n"
    markdown_output += (
        "### 💎 Want Access to the Elite Architect Class?\n"
    )
    markdown_output += (
        "You are currently viewing a complimentary public preview. To unlock"
        " full competitor gap matrices, deep audits, and direct team"
        " implementation, acquire a client pass at Trilloka."
    )

  # Save local report copy[cite: 2]
  filename_slug = (
  ...
  # 7. Background task: Dispatch Email Report[cite: 2]
  if request.email:
